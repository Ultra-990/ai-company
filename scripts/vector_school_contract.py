...
def validate_svg(source):
    ...
    if (root.text or '').strip() or not 11 <= len(root) <= 45:
    ...
    for node in root:
        ...
        for key, value in node.attrib.items():
            if key in NUMERIC:
                ...
                if key == 'font-size' and not 14 <= number <= 72:
    ...
    if len(texts) != 8 or len(set(texts)) != 8:
        raise ValueError('Exactly eight distinct editable text lines required')
    return {'texts': texts, 'elements': len(root), 'editable_text_count': len(texts),
            'shape_count': len(root) - len(texts), 'raster_images': 0}
...
def layout_issues(layout):
    ...
    for item in layout:
        ...
        if (not all(math.isfinite(v) for v in (x, y, width, height)) or width <= 0 or height <= 0
                or x < 12 or y < 12 or x + width > WIDTH - 12 or y + height > HEIGHT - 12):
