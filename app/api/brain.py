...
def safety_check(payload: SafetyCheckRequest, orchestrator: Orchestrator = Depends(get_orchestrator)) -> SafetyCheckResponse:
    """
    Wykonuje kontrolę bezpieczeństwa dla żądanej akcji.
    """
    result = orchestrator.run_safety_check(action_type=payload.action_type, requires_approval=payload.requires_approval)
    if hasattr(result, 'allowed') and hasattr(result, 'reason'):
        return SafetyCheckResponse(allowed=bool(result.allowed), reason=str(result.reason))
    if isinstance(result, dict):
        return SafetyCheckResponse.model_validate(result)
    raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail='Safety check returned an invalid result')
